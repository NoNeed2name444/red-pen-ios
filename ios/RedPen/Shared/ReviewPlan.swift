...
///
/// Pure on purpose - records in, decisions out - so it can be tested without a
/// phone or a clock.
enum ReviewPlan {
    ...
    /// What a rating leaves behind. The interval itself comes from
    /// AnkiScheduler, so the rating buttons' promised "in N days" and what is
    /// actually stored cannot drift apart.
    ...
    /// When the next card in a deck comes back, for a deck with nothing due.
    ...
    /// has cards waiting. This is the question actually being asked on the bus:
    /// what do I owe today?
    static func dueAcross(_ sets: [StudySet], records: [UUID: ReviewRecord],
                          now: Date = Date(), limit: Int = 500) -> [Due] {
        var found: [Due] = []
        for set in sets where set.kind == .anki {
            for card in set.cards {
                let kept = record(for: card, in: records, now: now)
                guard kept.due <= now else { continue }
                found.append(Due(setID: set.id, setName: set.name,
    ...
    /// by forgetting.
    static func pruned(_ records: [UUID: ReviewRecord],
                       keeping sets: [StudySet]) -> [UUID: ReviewRecord] {
        var live = Set<UUID>()
        for set in sets { for card in set.cards { live.insert(card.id) } }
