...
/// at 4-bit) once, then runs it fully on-device through llama.cpp — same
/// as the Apple path, this never touches anyone's Claude account or any
/// paid API, it's just a bigger one-time download and a slower model.
///
...
final class GemmaModel: ObservableObject {
    ...
    private static var isDownloaded: Bool {
        ...
        guard FileManager.default.fileExists(atPath: localURL.path, isDirectory: &isDir), !isDir.boolValue else { return false }
        ...
        return (size ?? 0) > Int64(Double(approxDownloadBytes) * 0.9)
    }
    ...
    func download() {
        ...
        downloadTask = Task {
            do {
                ...
                let tmpURL = Self.modelsDirectory.appendingPathComponent(Self.filename + ".part")
                try await Self.download(from: Self.downloadURL, to: tmpURL) { fraction in
                ...
                try FileManager.default.moveItem(at: tmpURL, to: Self.localURL)
